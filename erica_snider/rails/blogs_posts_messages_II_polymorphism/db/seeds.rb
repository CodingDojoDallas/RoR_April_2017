...
# rails g model Comments content:text commentable:references{polymorphic}

# add `has_many :comments, as: :commentable` to User, Blog, Post, and Message models

# Comment.create(content:"This is a comment on a message", commentable: Message.second)
# Message.all.joins(:comments).select("*")
# Comment.create(content:"This is a comment on a user", commentable: User.third)
# Comment.create(content:"This is a comment on a blog", commentable: Blog.fourth)
# Comment.create(content:"This is a comment on a post", commentable: Post.first)

# Post.joins(:comments).select("*")
# Post.find(1).comments
# Comment.where(commentable: User.all)
# Comment.where(commentable: User.third)
